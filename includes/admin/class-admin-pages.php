 ...
 * Управляет административными страницами.
 * Исправлено: При добавлении сигнатуры принудительно отправляется статистика (даже для бесплатных).
...
class RLS_Admin_Pages {
    ...
    // --- AJAX: Обезвреживание (с белым списком и AI) ---
    public function ajax_neutralize_file() {
        ...
        // 1. Проверка по WP Checksums (Ядро)
        if (isset($checksums[$relative_path])) {
            if (md5_file($filepath) === $checksums[$relative_path]) { 
                $this->add_to_whitelist($filepath, $checksums[$relative_path]); 
                wp_send_json_success(['result' => 'whitelisted', 'message' => 'Файл ядра WordPress.']); 
            }
        }
        ...
        if ($verdict === 'Legitimate') { 
            $this->add_to_whitelist($filepath, md5_file($filepath)); 
            wp_send_json_success(['result' => 'ai_legitimate', 'message' => 'AI счел код безопасным.']); 
        } else { 
            RLS_Stats_Helper::increment_stat('firewall_blocked'); 
            wp_send_json_success(['result' => 'ai_virus', 'message' => 'AI подтвердил вредоносный код.', 'snippet' => esc_html($code_snippet)]); 
        }
    }
    
    private function add_to_whitelist($filepath, $hash) { 
        $whitelist = get_option('rls_whitelist', []); 
        $whitelist[$filepath] = $hash; 
    ...
    // --- AJAX: Добавление сигнатуры (И ОТПРАВКА СТАТИСТИКИ) ---
    public function ajax_add_signature() { 
        ...
        if (empty($signature)) wp_send_json_error('Сигнатура пуста.'); 
        
        $custom_signatures = get_option('rls_custom_signatures', []); 
        if (!in_array($signature, $custom_signatures)) { 
            $custom_signatures[] = $signature; 
            update_option('rls_custom_signatures', $custom_signatures); 
            
            try { 
                // 1. Отправляем сам образец вируса
                RLS_API_Client::submit_suggestion($signature); 
                
                // 2. ВАЖНО: Принудительно отправляем статистику (для бесплатников тоже)
                // Это сбрасывает локальные счетчики и обновляет данные на сервере
